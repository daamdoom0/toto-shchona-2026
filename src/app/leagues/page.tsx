...
export default async function LeaguesPage() {
  ...
  const myLeagues = (memberships ?? [])
    .map((m) => m.leagues)
    .filter(Boolean) as { id: string; name: string; code: string; image_url: string | null; creator_id: string }[];
                ...
                <Link key={league.id} href={`/leagues/${league.id}`} className="toto-card block hover:shadow-lg transition-shadow">
                  <div className="flex items-center gap-4 p-4">
                    {league.image_url ? (
                      ...
                      <div className="w-16 h-16 rounded-lg bg-toto-green flex items-center justify-center text-3xl">🏆</div>
                    )}
                    <div className="flex-1 min-w-0">
                      <div className="font-display text-lg truncate">{league.name}</div>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="font-mono text-sm bg-toto-paper border border-toto-ink/20 px-2 py-0.5 rounded text-toto-ink/70">
                          {league.code}
                        </span>
